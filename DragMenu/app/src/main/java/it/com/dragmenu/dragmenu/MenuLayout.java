    ...
    private ViewDragHelper.Callback callback = new ViewDragHelper.Callback() {
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            if (changedView == mMenuView) {
                ...
                int currentTop = getRealTop();
                mMainView.layout(newLeft, currentTop, newLeft + mMainView.getMeasuredWidth(),
                        currentTop + mMainView.getMeasuredHeight());
            }
...
//            Log.i(TAG, "--------------------------fraction: " + fraction);
    ...
    public void openMenu() {
        mViewDragHelper.smoothSlideViewTo(mMainView, mDragRange, getRealTop());
        ViewCompat.postInvalidateOnAnimation(MenuLayout.this);
    }

    public void closeMenu() {
        mViewDragHelper.smoothSlideViewTo(mMainView, 0, getRealTop());
        ViewCompat.postInvalidateOnAnimation(MenuLayout.this);
    }
...
//        ViewHelper.setAlpha(mMenuView, mFloatEvaluator.evaluate(fraction, 0.5f, 1.0f));
    ...
    /**
     * 获取当前mMainView的真实top值,
     *
     * @return
     */
    public int getRealTop() {
        return ConstantUtil.getStatusHeight() == 0 ? mMainView.getTop() : ConstantUtil.getStatusHeight();
    }
}
